...
export default function PaymentSummary() {
  ...
  const createTicket = async (e) => {
    ...
    // Update ticket details with form inputs (name, email, etc.)
    ...
    try {
      ...
      // Attach QR code to ticket details
      updatedTicketDetails.ticketDetails.qr = qrCode;

      // Send the updated ticket details to the backend
    ...
    <>
      ...
      <div className="ml-12 bg-gray-100 shadow-lg mt-8 p-16 w-3/5 float-left">
        ...
        <div className="mt-10 space-y-4">
          ...
          <div className="float-right">
            <p className="text-sm font-semibold pb-2 pt-8">Total : LKR. {event.ticket_price}</p>
            <Link to={'/'}>
              <button type="button"
      ...
      <div className="float-right bg-blue-100 w-1/4 p-5 mt-8 mr-12">
        ...
        <div className="space-y-1">
          ...
          <p className="text-xs">{event.event_date.split("T")[0]},</p>
          <p className="text-xs pb-2"> {event.event_time}</p>
          <hr className=" my-2 border-t pt-2 border-gray-400" />
          <p className="pb-1 text-md">Ticket Price : LKR. {event.ticket_price}</p>
          <hr className=" my-2 border-t pt-2 border-gray-400" />
        </div>
      </div>
    </>
